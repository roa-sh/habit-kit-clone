  ...
  return (
    ...
    >
      <motion.div
        ...
        className="w-full bg-ios-card rounded-t-[32px] max-h-[85vh] overflow-hidden"
      ...
      >
        ...
        <div className="overflow-y-auto p-6 space-y-8" style={{ maxHeight: 'calc(85vh - 88px)' }}>
          {Object.entries(EMOJI_CATEGORIES).map(([category, emojis]) => (
            <div key={category}>
              <h4 className="text-xs font-bold text-ios-text-muted uppercase tracking-wide mb-4">
                {category}
              </h4>
                ...
                {emojis.map((emoji) => (
                  <motion.button
                    ...
                    className={`aspect-square rounded-2xl flex items-center justify-center text-3xl transition-all ${
                      emoji === currentEmoji
                        ? 'bg-habit-purple/30'
                        : 'bg-ios-card-secondary hover:bg-ios-border'
                    }`}
